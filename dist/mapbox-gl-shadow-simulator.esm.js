...
((i.prototype = {
    ...
    distanceTo: function (t) {
        var e = (t = a(t)).x - this.x, r = t.y - this.y;
    ...
    (s.prototype = {
        extend: function (t) {
            ...
            if (!t)
                return this;
            if (t instanceof i || "number" == typeof t[0] || "x" in t)
                e = r = a(t);
            else if (((e = (t = u(t)).min), (r = t.max), !e || !r))
                return this;
        ...
        contains: function (t) {
            var e, r;
            return ((t = "number" == typeof t[0] || t instanceof i ? a(t) : u(t)) instanceof
                s
                ? ((e = t.min), (r = t.max))
                : (e = r = t),
                e.x >= this.min.x &&
        ...
        pad: function (t) {
            var e = this.min, r = this.max, o = Math.abs(e.x - r.x) * t, i = Math.abs(e.y - r.y) * t;
            return u(a(e.x - o, e.y - i), a(r.x + o, r.y + i));
        },
        equals: function (t) {
            return (!!t &&
                ((t = u(t)),
    ...
    (l.prototype = {
        extend: function (t) {
            ...
            if (t instanceof f)
                ...
                if (!(t instanceof l))
                    return t ? this.extend(d(t) || h(t)) : this;
                if (((e = t._southWest), (r = t._northEast), !e || !r))
                    return this;
            }
            return (o || i
                ...
                : ((this._southWest = new f(e.lat, e.lng)),
                    (this._northEast = new f(r.lat, r.lng))),
                this);
        },
        pad: function (t) {
            var e = this._southWest, r = this._northEast, o = Math.abs(e.lat - r.lat) * t, i = Math.abs(e.lng - r.lng) * t;
            return new l(new f(e.lat - o, e.lng - i), new f(r.lat + o, r.lng + i));
        },
        getCenter: function () {
        ...
        contains: function (t) {
            t = "number" == typeof t[0] || t instanceof f || "lat" in t ? d(t) : h(t);
            var e, r, o = this._southWest, i = this._northEast;
...
var c = (function (t, ...args) {
    ...
    latLngToPoint: function (t, e) {
        var r = this.projection.project(t), o = this.scale(e);
        return this.transformation._transform(r, o);
    },
    pointToLatLng: function (t, e) {
        var r = this.scale(e), o = this.transformation.untransform(t, r);
    ...
    getProjectedBounds: function (t) {
        ...
        return new s(this.transformation.transform(e.min, r), this.transformation.transform(e.max, r));
    },
    infinite: false,
    ...
    wrapLatLngBounds: function (t) {
        var e = t.getCenter(), r = this.wrapLatLng(e), o = e.lat - r.lat, i = e.lng - r.lng;
        if (0 === o && 0 === i)
            return t;
        var n = t.getSouthWest(), a = t.getNorthEast();
        return new l(new f(n.lat - o, n.lng - i), new f(a.lat - o, a.lng - i));
    },
}, {
    wrapLng: [-180, 180],
    R: 6371e3,
    distance: function (t, e) {
        var r = Math.PI / 180, o = t.lat * r, i = e.lat * r, n = Math.sin(((e.lat - t.lat) * r) / 2), a = Math.sin(((e.lng - t.lng) * r) / 2), s = n * n + Math.cos(o) * Math.cos(i) * a * a, u = 2 * Math.atan2(Math.sqrt(s), Math.sqrt(1 - s));
        return this.R * u;
    },
});
function f(t, e, r) {
...
f.prototype = {
    ...
    distanceTo: function (t) {
        return c.distance(this, d(t));
    },
    wrap: function () {
        return c.wrapLatLng(this);
    },
    toBounds: function (t) {
        var e = (180 * t) / 40075017, r = e / Math.cos((Math.PI / 180) * this.lat);
        return h([this.lat - e, this.lng - r], [this.lat + e, this.lng + r]);
    },
    clone: function () {
...
// ============================================
const m = 256, _ = 0.40909994067971484, g = (t) => {
    const e = t.valueOf();
    return p(e);
}, p = (t) => {
    const e = t / 864e5 - 10957.5, r = 6.240059966692059 + 0.017201969994578018 * e, o = r +
        0.017453292519943295 *
            (1.9148 * Math.sin(r) +
                0.02 * Math.sin(2 * r) +
                3e-4 * Math.sin(3 * r)) +
        1.796593062783907 +
        Math.PI, i = Math.atan2(Math.sin(o) * Math.cos(_), Math.cos(o));
    return {
        dec: Math.asin(Math.sin(_) * Math.sin(o)),
        Hi: ((4.889714432387314 + 6.3003876824396166 * e - i) % (2 * Math.PI)) +
            2 * Math.PI,
    };
}, x = () => Math.floor(1e7 * Math.random()), E = (t, e, r) => {
    const o = 1 / e, i = Math.min(t[0] * o, 255), n = Math.min(t[1] * o, 255), a = Math.min(t[2] * o, 255);
    let s = 0;
    return (i + n + a !== 0 &&
        (s = i > 0 ? (i / 255) * 0.5 + 0.5 : a > 0 ? 0.5 * (1 - a / 255) : 0.5),
        s * r);
}, v = (t = new Date(), e) => {
    const r = new Date(t.toLocaleString("en-US", { timeZone: "UTC" })), o = new Date(t.toLocaleString("en-US", e ? { timeZone: e } : {}));
...
const T = (t) => {
    ...
    return {
        project: (r, o) => {
            if (e())
                return t.project(r, o);
            {
                const { lat: t, lng: e } = r;
                return new i(((t, e) => ((t + 180) / 360) * Math.pow(2, e) * m)(e, o), ((t, e) => ((1 -
                    Math.log(Math.tan((t * Math.PI) / 180) +
        ...
        unproject: (r, o) => {
            return e()
                ...
                : new f(((i = r.y),
                    (n = o),
                    (a = Math.PI - (2 * Math.PI * i) / m / Math.pow(2, n)),
                    (180 / Math.PI) * Math.atan(0.5 * (Math.exp(a) - Math.exp(-a)))), (function (t, e) {
                    return (t / m / Math.pow(2, e)) * 360 - 180;
                })(r.x, o));
            var i, n, a;
        },
        screenUnproject: (r) => e() ? t.containerPointToLatLng(r) : t.unproject(r),
...
class EventEmitter {
    ...
    removeAllListeners() {
        Object.keys(this.events).forEach((eventName) => this.events[eventName].splice(0, this.events[eventName].length));
    }
    emit(eventName, ...args) {
...
function A(t, e, r) {
    r = r || 2;
    var o, i, n, a, s, u, l, h = e && e.length, c = h ? e[0] * r : t.length, f = b(t, 0, c, r, true), d = [];
    if (!f || f.next === f.prev)
        return d;
    if ((h &&
        (f = (function (t, e, r, o) {
...
function M(t, e, r, o, i, n, a) {
    if (t) {
        !a &&
            ...
            (function (t, e, r, o) {
                ...
                ((i.prevZ.nextZ = null),
                    (i.prevZ = null),
                    (function (t) {
                        var e, r, o, i, n, a, s, u, l = 1;
                        do {
                            for (r = t, t = null, n = null, a = 0; r;) {
                                for (a++, o = r, s = 0, e = 0; e < l && (s++, (o = o.nextZ)); e++)
                                    ;
                                for (u = l; s > 0 || (u > 0 && o);)
                                    (0 !== s && (0 === u || !o || r.z <= o.z)
                                        ? ((i = r), (r = r.nextZ), s--)
        ...
        for (var s, u, l = t; t.prev !== t.next;)
            if (((s = t.prev), (u = t.next), n ? F(t, o, i, n) : w(t)))
                (e.push((s.i / r) | 0),
                    e.push((t.i / r) | 0),
                    e.push((u.i / r) | 0),
                    Z(t),
...
function w(t) {
    var e = t.prev, r = t, o = t.next;
    if (O(e, r, o) >= 0)
        return false;
    for (var i = e.x, n = r.x, a = o.x, s = e.y, u = r.y, l = o.y, h = i < n ? (i < a ? i : a) : n < a ? n : a, c = s < u ? (s < l ? s : l) : u < l ? u : l, f = i > n ? (i > a ? i : a) : n > a ? n : a, d = s > u ? (s > l ? s : l) : u > l ? u : l, m = o.next; m !== e;) {
        if (m.x >= h &&
            m.x <= f &&
            m.y >= c &&
            m.y <= d &&
...
function F(t, e, r, o) {
    var i = t.prev, n = t, a = t.next;
    if (O(i, n, a) >= 0)
        return false;
    for (var s = i.x, u = n.x, l = a.x, h = i.y, c = n.y, f = a.y, d = s < u ? (s < l ? s : l) : u < l ? u : l, m = h < c ? (h < f ? h : f) : c < f ? c : f, _ = s > u ? (s > l ? s : l) : u > l ? u : l, g = h > c ? (h > f ? h : f) : c > f ? c : f, p = C(d, m, e, r, o), x = C(_, g, e, r, o), E = t.prevZ, v = t.nextZ; E && E.z >= p && v && v.z <= x;) {
        if (E.x >= d &&
            E.x <= _ &&
            E.y >= m &&
            E.y <= g &&
    ...
    var r = (function (t, e) {
        ...
        do {
            (i >= o.x &&
                o.x >= h &&
                i !== o.x &&
                B(n < c ? i : a, n, h, c, n < c ? a : i, n, o.x, o.y) &&
                ((u = Math.abs(n - o.y) / (i - o.x)),
                    W(o, t) &&
                        (u < f || (u === f && (o.x > r.x || (o.x === r.x && P(r, o))))) &&
                        ((r = o), (f = u))),
                (o = o.next));
        } while (o !== l);
        return r;
...
function B(t, e, r, o, i, n, a, s) {
    return ((i - a) * (e - s) >= (t - a) * (n - s) &&
        (t - a) * (o - s) >= (r - a) * (e - s) &&
        (r - a) * (n - s) >= (i - a) * (o - s));
}
function N(t, e) {
    return (t.next.i !== e.i &&
        ...
        ((W(t, e) &&
            W(e, t) &&
            (function (t, e) {
                var r = t, o = false, i = (t.x + e.x) / 2, n = (t.y + e.y) / 2;
                do {
                    (r.y > n != r.next.y > n &&
                        r.next.y !== r.y &&
            ...
            (z(t, e) && O(t.prev, t, t.next) > 0 && O(e.prev, e, e.next) > 0)));
}
function O(t, e, r) {
...
function X(t, e, r, o) {
    var i = H(O(t, e, r)), n = H(O(t, e, o)), a = H(O(r, o, t)), s = H(O(r, o, e));
    return ((i !== n && a !== s) ||
        !(0 !== i || !G(t, r, e)) ||
        !(0 !== n || !G(t, o, e)) ||
        !(0 !== a || !G(r, t, o)) ||
...
function j(t, e) {
    var r = new K(t.i, t.x, t.y), o = new K(e.i, e.x, e.y), i = t.next, n = e.prev;
    return ((t.next = e),
        (e.prev = t),
        (r.next = i),
        (i.prev = r),
...
function V(t) {
    ...
    return t
        ...
        .map((t) => {
        const { geometry: e, properties: r } = t, o = A.flatten(e.coordinates), i = new Float32Array(o.vertices.map((t, e) => {
            return e % 2 == 1
                ? ((r = t),
                    (180 -
                        (180 / Math.PI) *
                ...
                : (180 + t) / 360;
            var r;
        })), n = A(o.vertices, o.holes, o.dimensions), a = n.length > 256 ? new Uint16Array(n) : new Uint8Array(n), s = (function (t) {
            const { height: e = 0, levels: r = 0, render_height: o = 0 } = t;
            if (r)
                return 3.04 * r;
            return o || e;
        })(r), u = r.highlight || false;
        let l = 0, h = 0, c = 0;
...
((A.deviation = function (t, e, r, o) {
    var i = e && e.length, n = i ? e[0] * r : t.length, a = Math.abs(k(t, 0, n, r));
    if (i)
        for (var s = 0, u = e.length; s < u; s++) {
            var l = e[s] * r, h = s < u - 1 ? e[s + 1] * r : t.length;
            a -= Math.abs(k(t, l, h, r));
        }
    var c = 0;
    for (s = 0; s < o.length; s += 3) {
        var f = o[s] * r, d = o[s + 1] * r, m = o[s + 2] * r;
        c += Math.abs((t[f] - t[m]) * (t[d + 1] - t[f + 1]) -
            (t[f] - t[d]) * (t[m + 1] - t[f + 1]));
    }
    return 0 === a && 0 === c ? 0 : Math.abs((c - a) / a);
}),
    (A.flatten = function (t) {
        for (var e = t[0][0].length, r = { vertices: [], holes: [], dimensions: e }, o = 0, i = 0; i < t.length; i++) {
            for (var n = 0; n < t[i].length; n++)
                for (var a = 0; a < e; a++)
                    r.vertices.push(t[i][n][a]);
...
const q = (t, e) => {
    const { lat: r, lng: o } = t;
    return new i(((t, e) => ((t + 180) / 360) * Math.pow(2, e) * m)(o, e), ((t, e) => ((1 -
        Math.log(Math.tan((t * Math.PI) / 180) + 1 / Math.cos((t * Math.PI) / 180)) /
            Math.PI) /
        2) *
        Math.pow(2, e) *
        m)(r, e));
    ...
    return new f(((r = t.y),
        (o = e),
        (i = Math.PI - (2 * Math.PI * r) / m / Math.pow(2, o)),
        (180 / Math.PI) * Math.atan(0.5 * (Math.exp(i) - Math.exp(-i)))), (function (t, e) {
        return (t / m / Math.pow(2, e)) * 360 - 180;
    })(t.x, e));
    var r, o, i;
};
let J, Q = {
    ...
    width: 0,
    height: 0,
    visibleDEMPixelBounds: new s(new i(0, 0), new i(0, 0)),
    DEMPixelBounds: new s(new i(0, 0), new i(0, 0)),
    maxHeight: 8848,
    raster: [],
    demZoom: 0,
    dirty: false,
...
const tt = async (e) => {
    const { demZoom: r, getFeatures: o, terrainSource: n, canopySource: a, dsmSource: u, tileLoaded: h, gl: c, bounds: f, buildingRasterizer: d, tileMerger: _, canopyMerger: g, forceUpdate: p = false, } = e, { getSourceUrl: x, getElevation: E, maxZoom: v, tileSize: T, _overzoom: y, } = n, A = ((b = { getFeatures: o }),
        (async () => {
    ...
    var b;
    try {
        const t = f, e = t.getNorthWest(), o = t.getSouthEast(), n = new s(q(e, r), q(o, r));
        let y = new i(n.min.x, n.min.y), b = n.max.x - n.min.x;
        const R = n.max.y - n.min.y, M = n.max.subtract(n.min);
        M.y > M.x && ((y.x -= m), (b += 512));
        const w = ((t) => {
            const { upperLeft: e, width: r, height: o } = t, i = e.divideBy(m).floor().multiplyBy(m), n = (Math.ceil(r / m) + 1) * m, a = (Math.ceil(o / m) + 1) * m, u = i.add([n, a]);
            return new s(i, u);
        })({ upperLeft: y, width: b, height: R }), { x: F, y: L } = w.max.subtract(w.min), U = ((t) => {
            const { upperLeft: e, width: r, height: o, zoom: i } = t, n = e.divideBy(m).floor(), a = n.x + r / m, s = Math.min(n.y + o / m, Math.pow(2, i) - 1), u = [];
            for (var l = n.x; l < a; l++)
                for (var h = n.y; h < s; h++)
                    u.push({ x: l, y: h, z: i });
            return u;
        })({ upperLeft: w.min, width: F, height: L, zoom: r }), D = ((t) => {
        ...
        })(U), S = n.max.x - n.min.x, P = n.max.y - n.min.y, C = Math.round(S), I = Math.round(P), B = JSON.stringify(D);
        if (!p && B === J && r < v)
            return ((Q = Object.assign(Object.assign({}, Q), {
                outputWidth: C,
                outputHeight: I,
        ...
        if (void 0 !== a) {
            ...
            N.push(new Promise((t) => {
                const e = c.createTexture();
                (c.activeTexture(c.TEXTURE2),
                    c.bindTexture(c.TEXTURE_2D, e),
                    c.texImage2D(c.TEXTURE_2D, 0, c.RGBA, 1, 1, 0, c.RGBA, c.UNSIGNED_BYTE, new Uint8ClampedArray([0, 0, 0, 0])),
                    t(e));
            }));
        ...
        if (null === O || null === z)
            return ((Q = Object.assign(Object.assign({}, Q), {
                ...
                Q);
        const X = new s(q(new l(u.bounds).getNorthWest(), r), q(new l(u.bounds).getSouthEast(), r)), G = X.min.subtract(w.min), H = X.max.subtract(w.min), W = [G.x / F, G.y / L, H.x / F, H.y / L].map((t) => 2 * t - 1), j = await A, { maxHeight: Y, heightMapTex: Z } = d.raster({
            upperLeftTile: D[0],
            width: F,
            height: L,
            mapZoom: r,
...
const et = (t, e) => {
    ...
    const { date: r } = e, { dec: o, Hi: i } = g(r);
    t.updateDate({ dec: o, Hi: i });
}, ot = async (e, r) => {
    return await e.updateDateRange(Object.assign({}, r));
}, it = (t, e) => {
    const { color: r, opacity: o } = e, i = et(r, o);
    t.updateColor({ colorVec: i });
}, nt = (t) => {
    const { kernel: e, map: r, heightMap: o, now: i, color: n, opacity: a, belowCanopy: s, skipRender: u, } = t;
    try {
        const { heightMapTex: t, outputHeight: h, outputWidth: c, maxHeight: f, width: d, height: _, DEMPixelBounds: p, visibleDEMPixelBounds: x, demZoom: E, } = o;
        if (0 === d || 0 === _)
            return;
        const { min: v, max: T } = x;
        if (!v || !T)
            return;
        const { x: y, y: A } = T.subtract(v), b = p.min, R = r.getPixelDimensions(), M = r.screenUnproject([0, 0]), w = r.screenUnproject([R.width, 0]), F = r.screenUnproject([R.width, R.height]), L = r.screenUnproject([0, R.height]), U = [L, F, M, w].map((t) => r.project(t, E)), D = U.map((t) => [(t.x - b.x) / d, (t.y - b.y) / _]).flat(), S = U.map((t) => [(t.x - v.x) / y, (t.y - v.y) / A])
            .map((t) => [2 * t[0] - 1, (2 * t[1] - 1) * (r.isLeaflet() ? -1 : 1)])
            .flat(), P = b.y / m / Math.pow(2, E), C = _ / m / Math.pow(2, E), I = new l($(p.getTopLeft(), E), $(p.getBottomRight(), E)), B = I.getWest(), N = Math.abs(I.getWest() - I.getEast());
        ((t, e) => {
            const { color: r, opacity: o, date: i } = e, n = et(r, o), { dec: a, Hi: s } = g(i);
            t.updateHeightMap(Object.assign({ dec: a, Hi: s, colorVec: n, step: 1 }, e));
        })(e, {
            heightMapTex: t,
            maxHeight: f,
            width: d,
...
class ShadeMapBase extends EventEmitter {
    constructor(options) {
        (super(),
            (this.options = {
                ...
                terrainSource: {
                    ...
                    tileSize: 256,
                    _overzoom: 15,
                    getSourceUrl: (t) => "https://s3.amazonaws.com/elevation-tiles-prod/terrarium/7/17/45.png",
                    getElevation: (t) => {
                        const { r: e, g: r, b: o } = t;
        ...
        (fetch("https://shademap.app/sdk/load", {
            ...
            .catch(async (e) => {
            throw new Error(`Could not load ShadeMap API. Key: ${this.options.apiKey} Error: ${e}`);
        }),
    ...
    setDate(date) {
        return (this.options.date.getTime() !== date.getTime()
    ...
    _setDateForTimezone(t, e) {
        let r = 0;
        (e && (r = v(this.options.date) - v(this.options.date, e)),
    ...
    setBelowCanopy(t) {
        return ((this.options.belowCanopy = t),
            this._heightMap ? this._draw(this._heightMap) : this._reset(),
            this);
    }
    setTerrainSource(t) {
        return ((this.options.terrainSource = t),
            delete this._heightMap,
            this._reset(),
            this);
    }
    setCanopySource(t) {
        return ((this.options.canopySource = t),
            delete this._heightMap,
            this._reset(),
            this);
    }
    setDSMSource(t) {
        return ((this.options.dsmSource = t),
            delete this._heightMap,
            this._reset(),
            this);
    }
    async setSunExposure(enabled = false, options) {
        if (((this.options.sunExposure = Object.assign(Object.assign({}, this.options.sunExposure), { enabled })),
            options)) {
            const { startDate: t = new Date(), endDate: o = new Date(), iterations: i = 32, } = options;
        ...
        if (this._map && this._compiledKernel && this._heightMap) {
            if (false === enabled)
                ...
                const { startDate: t, endDate: e, iterations: r, } = this.options.sunExposure;
                if (!(t instanceof Date && e instanceof Date))
                    throw new Error("Start date or end date or both are missing");
                if (e.getTime() < t.getTime())
                    throw new Error("End date must come after the start date to calculate sun exposure");
                if (true ===
    ...
    _lngLatToTextureCoords(t) {
        if (this._heightMap) {
            const { DEMPixelBounds: e, demZoom: r, width: o, height: i, } = this._heightMap, n = e.min;
            return t.map((t) => {
                const e = q(t, r);
    ...
    _getBounds(t, e) {
        const { width: r, height: o } = this.options.getSize();
    ...
    _getDEMZoom(t) {
        const e = Math.min(this.options.terrainSource._overzoom ||
            this.options.terrainSource.maxZoom, t.getZoom());
        return Math.round(e);
    }
    async _reset() {
            ...
            try {
                if (!t.isLeaflet() && this._map.getPitch() > 45) {
                    ...
                    const r = this._map.style._sourceCaches["other:composite"]
                        ...
                        .reverse(), o = r.reduce((t, e) => Math.max(t, e.canonical.z), Number.MIN_SAFE_INTEGER), n = r.filter((t) => t.canonical.z === o).map((t) => t.canonical), [a, s, u, l] = n.reduce((t, e) => [
                        Math.min(t[0], e.x),
                        Math.min(t[1], e.y),
                        Math.max(t[2], e.x),
                        Math.max(t[3], e.y),
                        ...
                        Number.MIN_SAFE_INTEGER,
                    ]), h = $(new i(512 * a, 512 * s), o + 1), c = $(new i(512 * (u + 1), 512 * (l + 1)), o + 1);
    ...
    async _draw(e) {
        if ((this.options.debug("_draw()"),
            this._canvas && this._compiledKernel && this._map)) {
            console.log("[SHADEMAP_DEBUG]", JSON.stringify({
                phase: "draw:start",
                hasHeightMap: !!e,
    ...
    readPixel(t, e) {
        const r = new Uint8Array(4), o = window.innerHeight - e;
        return (this._gl &&
            this._gl.readPixels(t, o, 1, 1, this._gl.RGBA, this._gl.UNSIGNED_BYTE, r),
            r);
    }
    readPixels(t, e, r, o) {
    ...
    toGeoTiff() {
        const t = (t, e) => {
            const r = this.readPixels(0, 0, t, e), o = t * e * 1, i = 1 * t, n = (e - 1) * i, a = new Uint8Array(o), s = new Uint8Array(o);
            let u = 0;
            ...
            for (let t = 0; t < r.length; t += 4) {
                ...
                if (this.options.sunExposure.enabled) {
                    const o = r.subarray(t, t + 3), i = E(o, 0.5, u) / 1e3 / 60, n = Math.min(Math.floor(i / 6), 255);
                    e = new Uint8Array([n]);
                }
                else {
                    e =
            ...
            if (this._map && !T(this._map).isLeaflet())
                return a;
            for (let t = 0; t < o; t += i)
                s.set(a.subarray(t, t + i), n - t);
            return s;
        };
        if (this._map && this._heightMap) {
            const e = this._heightMap.outputWidth, r = this._heightMap.outputHeight, o = t(e, r), i = T(this._map), { lat: n, lng: a } = i.getBounds().getNorthWest(), s = i.getBounds().getSouthEast(), u = [0, 0, 0, a, n, 0], l = [(s.lng - a) / e, (n - s.lat) / r, 0];
            return (this.options.sunExposure.enabled,
                {
                    data: o,
                    metadata: {
                        width: e,
    ...
    _generateShadeProfile(t) {
        if (this._compiledKernel) {
            const e = this._lngLatToTextureCoords(t.locations), r = ((t, e) => {
                const r = et({ r: e.sunColor[0], g: e.sunColor[1], b: e.sunColor[2] }, 1), o = et({ r: e.shadeColor[0], g: e.shadeColor[1], b: e.shadeColor[2] }, 1);
                return t.generateShadeProfile(Object.assign(Object.assign({}, e), { sunColor: r, shadeColor: o }));
            })(this._compiledKernel, Object.assign(Object.assign({}, t), { texCoords: e }));
            return r;
        }
    ...
    _generateLocationShadeProfile(t) {
        if (this._compiledKernel) {
            const e = v(t.startDate) - v(t.startDate, t.tzId), r = t.startDate.getTime() - e, o = t.endDate.getTime() - e, i = this._lngLatToTextureCoords([t.location])[0], { output: n, outputWidth: a, outputHeight: s, } = ((t, e) => {
                const r = et({ r: e.sunColor[0], g: e.sunColor[1], b: e.sunColor[2] }, 1), o = et({ r: e.shadeColor[0], g: e.shadeColor[1], b: e.shadeColor[2] }, 1);
                return t.generateLocationShadeProfile(Object.assign(Object.assign({}, e), { sunColor: r, shadeColor: o }));
            })(this._compiledKernel, Object.assign(Object.assign({}, t), {
                startTime: r,
                endTime: o,
                texCoord: i,
            }));
            return ((n.toArray = function () {
                ...
                for (let r = 0; r < a; r++) {
                    const o = [];
                    for (let e = s - 1; e >= 0; e--) {
                        const i = e * a * 4 + 4 * r, n = this.slice(i, i + 4);
    ...
    getHoursOfSun(t, e) {
        if (this.options.sunExposure.enabled) {
            const r = this.readPixel(t, e), { startDate: o, endDate: i } = this.options.sunExposure, n = i.getTime() - o.getTime(), a = E(r, 0.5, n);
...
const st = (t) => {
    const { gl: e } = t, { texture: r, imageData: o = null, format: i = e.RGBA, width: n, height: a, filter: s = e.NEAREST, wrap: u = e.CLAMP_TO_EDGE, type: l = e.UNSIGNED_BYTE, internalFormat: h = i, } = t;
...
function ut(t) {
    const { gl: e, vSrc: r, fSrc: o } = t, i = e.createShader(e.VERTEX_SHADER);
...
class BuildingRasterizer {
    constructor(t) {
        ((this.gl = t),
            ...
            (this.positionAttributeLocation = t.getAttribLocation(this.program, "a_position")),
            (this.dsmAttributeLocation = t.getAttribLocation(this.program, "dsm_position")),
            (this.useDSMUniformLocation = t.getUniformLocation(this.program, "use_dsm")),
            (this.isNegative = t.getUniformLocation(this.program, "is_negative")),
            (this.xyzUniformLocation = t.getUniformLocation(this.program, "xyz")),
            (this.dimensionsUniformLocation = t.getUniformLocation(this.program, "dimensions")),
            (this.heightMapUniformLocation = t.getUniformLocation(this.program, "height_map")),
            (this.canopyMapUniformLocation = t.getUniformLocation(this.program, "canopy_map")),
            (this.centroidUniformLocation = t.getUniformLocation(this.program, "u_centroid")),
    ...
    raster(t) {
        const { features: e, upperLeftTile: r, mapZoom: o, width: n, height: a, imageData: s, dsmCoords: u, dsmSource: l, canopyData: h, gl: c, } = t, { x: f, y: d, z: _ } = r;
        let g = 0;
        ...
        (c.activeTexture(c.TEXTURE0),
            ...
            c.texImage2D(c.TEXTURE_2D, 0, c.RGBA, p, x, 0, c.RGBA, c.UNSIGNED_BYTE, null),
            o > 15 && e.length > 5
        ...
        (c.framebufferTexture2D(c.FRAMEBUFFER, v, c.TEXTURE_2D, this.targetTexture, 0),
            c.disable(c.BLEND),
        ...
        if ((c.enableVertexAttribArray(this.positionAttributeLocation),
            c.bufferData(c.ARRAY_BUFFER, M, c.STATIC_DRAW),
            c.vertexAttribPointer(this.positionAttributeLocation, 2, c.FLOAT, false, 0, 0),
            c.uniform4f(this.colorUniformLocation, 1, 1, 1, 1),
            c.drawArrays(c.TRIANGLE_STRIP, 0, 4),
            e
                .filter((t) => !t.highlight)
                .forEach((t) => {
                const { buildingHeight: e, aPosition: r, cuts: o, centroid: s, highlight: u, } = t, l = new i(s[0], s[1])
                    .subtract(A)
                    .unscaleBy(new i(b, R))
                    .scaleBy(new i(n, a))
                    .floor();
                if (l.x < 0 || l.y < 0 || l.x > n || l.y > a)
                    return;
                ...
                ((g = Math.max(g, e)),
                    ...
                    c.uniform1i(this.isNegative, e < 0 ? 1 : 0),
                    ...
                    c.bufferData(c.ELEMENT_ARRAY_BUFFER, o, c.DYNAMIC_DRAW),
                    c.drawElements(c.TRIANGLES, o.length, o.length > 256 ? c.UNSIGNED_SHORT : c.UNSIGNED_BYTE, 0));
            }),
            0 !== l.data.length)) {
            c.activeTexture(c.TEXTURE1);
            ...
            (c.bindTexture(c.TEXTURE_2D, t),
                ...
                c.texParameteri(c.TEXTURE_2D, c.TEXTURE_MIN_FILTER, c.NEAREST),
                c.texParameteri(c.TEXTURE_2D, c.TEXTURE_MAG_FILTER, c.NEAREST),
                c.texImage2D(c.TEXTURE_2D, 0, c.LUMINANCE_ALPHA, l.width, l.height, 0, c.LUMINANCE_ALPHA, c.UNSIGNED_BYTE, l.data),
            ...
            (c.enableVertexAttribArray(this.positionAttributeLocation),
                ...
                c.bufferData(c.ARRAY_BUFFER, new Float32Array([0, 1, 1, 1, 0, 0, 1, 0]), c.STATIC_DRAW),
                c.vertexAttribPointer(this.dsmAttributeLocation, 2, c.FLOAT, false, 0, 0),
                c.uniform4f(this.colorUniformLocation, 1, 1, 1, 1),
        ...
        return (c.colorMask(true, true, true, true),
            c.activeTexture(c.TEXTURE1),
            c.bindTexture(c.TEXTURE_2D, s),
            c.activeTexture(c.TEXTURE2),
            c.bindTexture(c.TEXTURE_2D, h),
...
const ht = (t) => {
    ...
    return {
        x: o === m ? i : Math.floor(i / 2),
        y: o === m ? n : Math.floor(n / 2),
        z: o === m ? a : a - 1,
    };
};
// ============================================
...
class TileMerger {
    constructor(t) {
        ((this.gl = t),
            ...
            (this.texPositionAttributeLocation = t.getAttribLocation(this.program, "a_tex_position")),
            (this.tilePositionAttributeLocation = t.getAttribLocation(this.program, "a_tile_position")),
            (this.encodingUniformLocation = t.getUniformLocation(this.program, "u_encoding")),
            (this.tileSizeUniformLocation = t.getUniformLocation(this.program, "u_tile_size")),
            ...
            t.vertexAttribPointer(this.texPositionAttributeLocation, 2, t.FLOAT, false, 0, 0),
    ...
    async merge(e, r) {
        ...
        if (o) {
            ...
            (o.bindFramebuffer(o.FRAMEBUFFER, _),
                o.framebufferTexture2D(o.FRAMEBUFFER, o.COLOR_ATTACHMENT0, o.TEXTURE_2D, this.outputTexture, 0));
                    ...
                    (this.inProgress.push(c),
                        (c.onload = () => {
                            (o.useProgram(this.program),
                                ...
                                o.texImage2D(o.TEXTURE_2D, 0, o.RGBA, o.RGBA, o.UNSIGNED_BYTE, c),
                                o.bindFramebuffer(o.FRAMEBUFFER, _));
                            ...
                            (o.bindBuffer(o.ARRAY_BUFFER, this.tilePositionBuffer),
                                ...
                                o.vertexAttribPointer(this.tilePositionAttributeLocation, 2, o.FLOAT, false, 0, 0));
                            let F = [0, 0, 1, 0, 0, 1, 1, 1];
                            const L = Math.pow(2, Math.max(0, y - f) + (h !== m ? 1 : 0)), U = 514 === h ? 1 / h : 0, D = 1 / L, S = (x % L) / L, P = (E % L) / L, C = (v % L) / L, I = (T % L) / L;
                            ((F = [S, P, C + D, P, S, I + D, C + D, I + D].map((t) => t * (514 === h ? 512 / 514 : 1) + U)),
                                ...
                                o.vertexAttribPointer(this.texPositionAttributeLocation, 2, o.FLOAT, false, 0, 0),
...
class MapboxShadeMap extends ShadeMapBase {
    ...
    onAdd(e, gl) {
        ...
        ((this._compiledKernel = (function (e) {
            ...
            let j = 0, Y = 0, Z = 0, K = "";
            const k = (t) => {
                const e = (t.endTime - t.startTime) / 86400 / 1e3, r = 1440;
                if ((i.activeTexture(i.TEXTURE3),
                    i.uniform1i(C, 3),
                    t.startTime !== Y || t.endTime !== Z || t.tzId !== K)) {
                    ((Y = t.startTime), (Z = t.endTime), (K = t.tzId));
                    const o = [], n = 828e5, a = v(new Date(Y), K);
                    for (let t = 0; t < e; t++) {
                        const e = v(new Date(Y + 86400 * t * 1e3 + n), K) - a;
                        o[t] = e;
                    }
                    const s = new Float32Array(r * e * 2);
                    for (let t = 0; t < r; t++)
                        for (let r = 0; r < e; r++) {
                            const i = Y + 86400 * r * 1e3 + 60 * t * 1e3, { dec: n, Hi: a } = p(i + o[r]);
                    ...
                    st({
                        ...
                        width: e,
                        height: r,
                        wrap: i.CLAMP_TO_EDGE,
                        filter: i.NEAREST,
                        format: i.RG,
                        internalFormat: i.RG32F,
                        type: i.FLOAT,
                    });
                }
                else
            ...
            return {
                updateHeightMap: function (t) {
                    const { heightMapTex: e, width: r, height: o, maxHeight: s, heightMapZoom: l, topYCoord: h, ySize: c, colorVec: f, step: g, west: p, dLng: v, dec: L, Hi: U, cornerClipCoords: D, cornerTextureCoords: S, outputWidth: P, outputHeight: C, belowCanopy: I, skipRender: B, } = t;
                    (i.useProgram(n),
                        (X = P),
                        (G = C),
                        (H = e),
                        i.bindBuffer(i.ARRAY_BUFFER, a),
                        i.bufferData(i.ARRAY_BUFFER, new Float32Array(D), i.STATIC_DRAW),
                        i.bindBuffer(i.ARRAY_BUFFER, u),
                ...
                updateDateRange: async (e) => {
                    ...
                    (i.activeTexture(i.TEXTURE2),
                        ...
                        i.texImage2D(i.TEXTURE_2D, 0, i.RGBA, m, _, 0, i.RGBA, i.UNSIGNED_BYTE, null),
                ...
                generateShadeProfile: (t) => {
                    ...
                    const v = e
                        .map((t) => {
                        const { dec: e, Hi: r } = g(t);
                        return [-e, r / 10];
                    })
                        .flat(), T = i.createTexture();
                    ...
                    return (i.readPixels(0, 0, u, f, i.RGBA, i.UNSIGNED_BYTE, y),
                        i.deleteTexture(m),
                        i.deleteTexture(T),
                        i.deleteTexture(E),
                        i.deleteFramebuffer(_),
                        i.uniform1i(C, 0),
                        i.uniform1i(P, 0),
                        i.uniform1i(S, 0),
                        i.uniform4fv(w, d),
                        i.uniform4fv(I, [0, 0, 0, 0]),
                        y);
                },
                generateLocationShadeProfile: (t) => {
                    i.useProgram(n);
                    const { startTime: e, endTime: r, tzId: o, texCoord: a, shadeColor: u, sunColor: f, panelAltitude: d, panelAzimuth: m, } = t, _ = i.getUniform(n, w), { outputWidth: g, outputHeight: p } = k({
                        startTime: e,
                        endTime: r,
                        tzId: o,
                    });
                    ...
                    (i.bindFramebuffer(i.FRAMEBUFFER, E),
                        i.framebufferTexture2D(i.FRAMEBUFFER, i.COLOR_ATTACHMENT0, i.TEXTURE_2D, x, 0));
                    const v = i.checkFramebufferStatus(i.FRAMEBUFFER);
                    (v !== i.FRAMEBUFFER_COMPLETE &&
                        console.error("Framebuffer is incomplete: " + v),
                        null !== H &&
                            (i.activeTexture(i.TEXTURE0), i.bindTexture(i.TEXTURE_2D, H)),
                        i.bindBuffer(i.ARRAY_BUFFER, h),
                        i.enableVertexAttribArray(s),
                    ...
                    return (i.readPixels(0, 0, g, p, i.RGBA, i.UNSIGNED_BYTE, T),
                        i.deleteTexture(x),
                        ...
                        i.uniform4fv(w, _),
                        i.uniform4fv(I, [0, 0, 0, 0]),
                        { output: T, outputWidth: g, outputHeight: p });
                },
            };
        })({
            context: this._gl,
        ...
        const r = e.getBounds(), o = r.getNorthWest(), i = r.getNorthEast(), n = r.getSouthEast(), a = r.getSouthWest(), s = [
            [o.lng, o.lat],
            [i.lng, i.lat],
            [n.lng, n.lat],
            [a.lng, a.lat],
        ];
        return (e.addSource(this.canvasSourceId, {
            type: "canvas",
            canvas: this._canvas,
            coordinates: s,
            animate: false,
            ...
            e.addSource(this.attributionSourceId, {
                type: "geojson",
                data: {
                    type: "Feature",
                    properties: {},
    ...
    _getHeightMapCoords(t, e) {
        if (this._map && this._bounds && this._heightMap) {
            ...
            return T(this._map).project(r, n).subtract(o.min);
        }
        return new i(-1, -1);
    }
    getHoursOfSun(t, e) {
        if (this.options.sunExposure.enabled) {
            const r = this._getHeightMapCoords(t, e), o = this.readPixel(r.x, r.y), { startDate: i, endDate: n } = this.options.sunExposure, a = n.getTime() - i.getTime(), s = E(o, 0.5, a);
    ...
    _repositionCanvas(t) {
        if (this._map) {
            ...
            if (e) {
                const r = t.getNorthWest(), o = t.getNorthEast(), i = t.getSouthEast(), n = t.getSouthWest(), a = [
                    [r.lng, r.lat],
                    [o.lng, o.lat],
                    [i.lng, i.lat],
                    [n.lng, n.lat],
